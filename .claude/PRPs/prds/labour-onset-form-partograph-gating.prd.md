...
| 4 | Gating na UI do modo parto | `birth-mode-screen.tsx`: desabilitar/ocultar tab Partograma e filtrar `events` passados a `BirthModePartograph` conforme `partograph_unlocked_at`; manter sincronismo via realtime | complete | - | 3 | `.claude/PRPs/plans/completed/labour-onset-form-partograph-gating-phase-4.plan.md` |
