-- Adicionar colunas de finalização na tabela patients
ALTER TABLE public.patients
  ADD COLUMN has_finished boolean NOT NULL DEFAULT false,
  ADD COLUMN born_at date;

-- Recriar a função get_filtered_patients com suporte ao filtro 'finished'
...
AS $$
  ...
  WHERE p.id = ANY(patient_ids)
    ...
    AND (p.has_finished = false OR filter_type = 'finished')
    AND (
      filter_type = 'all'
  ...
  ORDER BY
    ...
    CASE WHEN filter_type = 'finished' THEN p.born_at END DESC NULLS LAST,
    p.due_date ASC
  LIMIT page_limit
  OFFSET page_offset;
$$;
