-- Contract dual signature (Phase 8 follow-up): add finalized_content_hash — hash of
-- the merged (contract + certificate) PDF buffer, computed once at generation time.
-- Distinct from content_hash (the original, professional-only PDF's hash), since the
-- pdf-lib merge is not byte-deterministic and the finalized document is now the one
-- users actually download and verify at /check/[codigo].
ALTER TABLE public.contracts
  ADD COLUMN finalized_content_hash text;
...
BEGIN
  IF (OLD.title IS DISTINCT FROM NEW.title)
     ...
     -- finalized_content_hash may only transition once, from NULL to a value — set
     -- together with finalized_document_id by sign-contract-as-patient-action.ts
     OR (OLD.finalized_content_hash IS NOT NULL AND OLD.finalized_content_hash IS DISTINCT FROM NEW.finalized_content_hash)
