...
const slides: SlideData[] = [
  {
    ...
    description: "Acompanhe cada gestante com todas as informações centralizadas em um só lugar.",
  },
  {
    image: welcomeSlide2,
    title: "Trabalhe em equipe",
    description: "Convide profissionais e colabore no cuidado de cada paciente em tempo real.",
  },
  {
    image: welcomeSlide3,
    title: "Fique por dentro de tudo",
    description: "Receba notificações e nunca perca um compromisso ou atualização importante.",
  },
];
...
export default function WelcomeScreen() {
  ...
  const { title, description } = (slides[displayStep] ?? FALLBACK_SLIDE) as SlideData;

  return (
    <div className="flex h-svh w-full flex-col bg-background">
      <div className="relative flex flex-1 flex-col items-center justify-center overflow-hidden py-8">
        <div className="w-full px-8">
          <Image src={ventreLogo} alt="logo" className="object-contain" width={140} height={140} />
        </div>
        ...
        <div className="flex h-full w-full justify-center overflow-hidden">
          ...
          >
            {slides.map(({ image, title: slideTitle }) => (
              <div key={slideTitle} className="flex w-full shrink-0 items-center justify-center">
          ...
          {slides.map((slide, dotIndex) => (
            <span
              ...
              style={{
                ...
                backgroundColor:
                  dotIndex === step ? "hsl(var(--primary))" : "hsl(var(--secondary))",
                transitionDuration: `${TRANSITION_MS}ms`,
              }}
            />
          ))}
