...
type PatientsHistoryEnterpriseScreenProps = {
...
export default function PatientsHistoryEnterpriseScreen({
  patients,
  totalCount,
  currentPage,
  initialSearch,
  ...
  teamMembersMap,
}: PatientsHistoryEnterpriseScreenProps) {
  ...
  const buildUrl = useCallback((search: string, profId: string | null, page = 1) => {
  ...
  const handleProfessionalSelect = (id: string) => {
    ...
    startTransition(() => router.push(buildUrl(searchQuery, newId)));
  };
  ...
  const handleSearchChange = (value: string) => {
    ...
    searchTimeoutRef.current = setTimeout(() => {
      router.push(buildUrl(value, professionalId));
    }, 400);
  };
              ...
              <Pagination className="mt-6">
                <PaginationContent>
                  {currentPage > 1 && (
                    <PaginationItem>
                      <PaginationPrevious
                        href={buildUrl(searchQuery, professionalId, currentPage - 1)}
                      />
                    </PaginationItem>
                  )}
                    ...
                    .map((page, idx, arr) => {
                      ...
                      items.push(
                        <PaginationItem key={page}>
                          <PaginationLink
                            href={buildUrl(searchQuery, professionalId, page)}
