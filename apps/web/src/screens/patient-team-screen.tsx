...
export default function PatientTeamScreen() {
	...
	const loading =
		["idle", "executing"].includes(patientStatus) ||
		["idle", "executing"].includes(teamStatus);
	const patientOwnerId = patient?.created_by;
	const currentUserMember = teamMembers.find(
		(m) => m.professional_id === user?.id,
	);

	if (loading) return <LoadingPatientTeam />;
	if (!patient) return null;

	const ROLE_ORDER: ProfessionalType[] = [
		"obstetra",
		"enfermeiro",
		"doula",
		"fisio",
	...
	const pendingInviteByType = Object.fromEntries(
		ROLE_ORDER.map((role) => [
			...
			rawInvites.find((inv) => {
				...
				const resolvedType =
					inv.professional_type ?? professional?.professional_type;
	...
	const usedTypes = teamMembers
		.filter((m) => !m.is_backup)
		.map((m) => m.professional_type);
