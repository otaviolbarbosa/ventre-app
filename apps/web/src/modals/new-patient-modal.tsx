...
function getBackupProfessionalIds(
	ids: string[],
...
export default function NewPatientModal({
	...
	const [dueDateCalcMethod, setDueDateCalcMethod] = useState<
		DueDateCalcMethod | undefined
	>(undefined);
	...
	const showEnterpriseSelector =
		enterprises !== undefined && !showProfessionalSelector;
	const step4Label = showEnterpriseSelector ? "Empresa" : "Equipe";

	const defaultProfessionalIds = professional?.id
		? [professional.id]
		: undefined;
	...
	const isSplitBilling =
		showProfessionalSelector && selectedProfessionalIds.length > 1;
	const splitBillingTotal = Object.values(profAmounts).reduce(
		(a, b) => a + b,
		0,
	);
	...
	const hasAmountMismatch =
		billingTotalAmount > 0 && installmentSum !== billingTotalAmount;
	...
	useEffect(() => {
		...
		const next = Array.from(
			{ length: billingInstallmentCount },
			(_, i) => current[i] ?? "",
		);
	...
	function applyDum(date: Date | null) {
		...
		form.setValue(
			"due_date",
			dayjs(dumStr).add(280, "day").format("YYYY-MM-DD"),
		);
	}
	...
	function applyDpp(date: Date | null) {
		...
		form.setValue(
			"dum",
			dayjs(dppStr).subtract(280, "day").format("YYYY-MM-DD"),
		);
	}
	...
	function onSubmit(data: CreatePatientInput) {
		...
		if (showBilling && data.billing) {
			...
			const hasEmptyInstallments =
				billingTotalAmount > 0 && installmentAmounts.some((a) => a <= 0);
		...
		if (showBilling && isSplitBilling && data.billing) {
			...
			if (Object.values(profAmounts).some((v) => v <= 0)) {
				toast.error(
					"Todos os valores das profissionais devem ser maiores que zero.",
				);
				return;
			}
			execute({
				...
				billing: {
					...
					installment_interval: isCustomInterval
						? null
						: data.billing.installment_interval,
							...
							<div className="space-y-4">
								...
								<div className="space-y-2">
									...
									<Select
										...
										onValueChange={(v) =>
											handleCalcMethodChange(v as DueDateCalcMethod)
										}
									>
										<SelectTrigger>
										...
										<SelectContent>
											<SelectItem value="gestational_age">
												Idade gestacional
											</SelectItem>
											...
											<SelectItem value="fiv">
												FIV/FET (transferência de embrião)
											</SelectItem>
										</SelectContent>
									</Select>
								</div>
										...
										<FormItem>
											...
											<FormControl>
												<Input
													...
													value={gestAgeWeeks}
													onChange={(e) =>
														applyGestAge(e.target.value, gestAgeDays)
													}
												/>
											</FormControl>
										</FormItem>
										<FormItem>
											...
											<FormControl>
												<Input
													...
													value={gestAgeDays}
													onChange={(e) =>
														applyGestAge(gestAgeWeeks, e.target.value)
													}
												/>
											</FormControl>
										</FormItem>
									...
									<div className="grid gap-4 sm:grid-cols-2">
										<FormItem>
											...
											<FormControl>
												<DatePicker
													selected={
														fivTransferDate
															? new Date(`${fivTransferDate}T00:00:00`)
															: null
													}
													onChange={(date) =>
														applyFiv(
										...
										<FormItem>
											...
											<Select
												...
												onValueChange={(v) =>
													applyFiv(fivTransferDate, v as FivTransferType)
												}
											>
												<FormControl>
													<SelectTrigger>
								...
								{showProfessionalSelector ? (
									<FormField
										...
										render={({ field, fieldState }) => {
											...
											const hasError =
												!!fieldState.error && selectedIds.length === 0;
											const selectedProfessionals = selectedIds
												.map((id) =>
													professionalsOptions.find((p) => p.id === id),
												)
														...
														<SearchableDropdown
															...
															options={professionalsOptions.map((prof) => ({
																...
																group: prof.professional_type
																	? PROFESSIONAL_TYPE_PLURAL_LABELS[
																			prof.professional_type
																		]
																	: undefined,
															}))}
										...
										<FormField
											...
											render={({ field }) => (
												<FormItem>
													...
													<FormControl>
														<Input
															placeholder="Ex: Acompanhamento pré-natal"
															{...field}
														/>
													</FormControl>
													<FormMessage />
										...
										{isSplitBilling ? (
											<div className="space-y-2">
												...
												{Object.keys(profAmounts).map((profId) => {
													const prof = professionalsOptions.find(
														(p) => p.id === profId,
													);
													return (
														<div
												...
												{selectedProfessionalIds.some(
													(id) => !(id in profAmounts),
												) && (
											...
											<FormItem>
												...
												<Select
													value={
														isCustomInterval
															? "custom"
															: String(billingInstallmentInterval ?? "")
													}
											...
											<div className="space-y-2">
													...
													<div className="flex items-start gap-3">
														...
														<div className="w-28 shrink-0">
															<CurrencyInput
																...
																onChange={(val) =>
																	handleInstallmentAmountChange(0, val)
																}
															/>
														...
														>
															...
															<div className="w-28 shrink-0">
																<CurrencyInput
																	...
																	onChange={(val) =>
																		handleInstallmentAmountChange(i + 1, val)
																	}
																/>
												...
												{hasAmountMismatch && (
													<p className="text-destructive text-xs">
														A soma das parcelas (
														{formatCurrency(installmentSum)}) não corresponde ao
														valor total ({formatCurrency(billingTotalAmount)}).
													</p>
												)}
											</div>
										)}
															...
															>
																...
																<div className="w-28 shrink-0">
																	<CurrencyInput
																		...
																		onChange={(val) =>
																			handleInstallmentAmountChange(i, val)
																		}
																	/>
