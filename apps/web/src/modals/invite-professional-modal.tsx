...
export default function InviteProfessionalModal({
	...
	const { executeAsync: executeInviteDirect, isPending: isInviting } =
		useAction(inviteProfessionalDirectAction);
	const { executeAsync: executeInviteLink, isPending: isLinkPending } =
		useAction(createTeamMemberInviteAction);
	const { executeAsync: executeSearch, isPending: isLoading } =
		useAction(searchUsersAction);
