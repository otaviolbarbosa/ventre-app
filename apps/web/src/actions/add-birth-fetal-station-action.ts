...
import { birthFetalStationSchema } from "@/lib/validations/birth-mode";
import { z } from "zod";

const schema = z.object({
  pregnancyId: z.string().uuid(),
  data: birthFetalStationSchema,
});

export const addBirthFetalStationAction = authActionClient
  .inputSchema(schema)
  .action(async ({ parsedInput, ctx: { supabase, user } }) => {
    ...
    const { error } = await supabase.from("birth_fetal_stations").insert({
    ...
    await captureServerEvent(user.id, "add_birth_fetal_station", { pregnancy_id: pregnancyId });
