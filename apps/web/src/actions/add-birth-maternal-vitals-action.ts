...
import { birthMaternalVitalsSchema } from "@/lib/validations/birth-mode";
import { z } from "zod";

const schema = z.object({
  pregnancyId: z.string().uuid(),
  data: birthMaternalVitalsSchema,
});

export const addBirthMaternalVitalsAction = authActionClient
  .inputSchema(schema)
  .action(async ({ parsedInput, ctx: { supabase, user } }) => {
    ...
    const { error } = await supabase.from("birth_maternal_vitals").insert({
    ...
    await captureServerEvent(user.id, "add_birth_maternal_vitals", { pregnancy_id: pregnancyId });
