...
export async function sendPatientInvite({
  ...
  const { error } = await resend.emails.send({
    ...
    subject: enterpriseName
      ? `${enterpriseName} te convidou para o Ventre`
      : "Complete seu cadastro no Ventre",
    html: `
<!DOCTYPE html>
<html lang="pt-BR">
          ...
          <tr>
            <td style="background-color:#ffffff;border-radius:16px;padding:48px 40px;box-shadow:0 1px 4px rgba(0,0,0,0.06);">
              ...
              <p style="margin:0 0 24px;font-size:15px;line-height:1.7;color:#81726C;text-align:center;">
                Olá, <strong style="color:#433831;">${name}</strong>! Sua equipe de acompanhamento pré-natal te convidou para criar sua conta no <strong style="color:#1a1a1a;">Ventre</strong>${
                  enterpriseName
                    ? ` na <strong style="color:#433831;">${enterpriseName}</strong>`
                    : ""
                }. Clique no botão abaixo para finalizar seu cadastro e acompanhar sua agenda, cartão pré-natal e financeiro.
              </p>
