...
describe("computeUterineActivityChartColumns", () => {
  it("decompõe 20min em 2 colunas de 3 células cada (exemplo do requisito), preenchendo até 10 colunas", () => {
    ...
    expect(columns.slice(2)).toEqual(Array.from({ length: 8 }, () => ({ cells: [] })));
  });
  ...
  it("retorna coluna com array de células vazio quando todas as contrações do bloco são <20s", () => {
    ...
    expect(columns).toEqual(Array.from({ length: 10 }, () => ({ cells: [] })));
  });

  it("preenche até 10 colunas quando não há registros", () => {
    const columns = computeUterineActivityChartColumns([]);
    expect(columns).toEqual(Array.from({ length: 10 }, () => ({ cells: [] })));
  });

  it("não corta colunas quando há mais de 10 blocos", () => {
    const columns = computeUterineActivityChartColumns([
