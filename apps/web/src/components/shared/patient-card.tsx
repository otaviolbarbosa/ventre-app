...
export function PatientCard({
  ...
  const patientWeeks: number = patient.dum ? (calculateGestationalAge(patient.dum)?.weeks ?? 0) : 0;
  const dppFormatted = patient.due_date ? dayjs(patient.due_date).format("DD/MM") : null;
  const statusColor =
    patientWeeks && patientWeeks >= 37 ? "#802f2d" : patientWeeks >= 28 ? "#cc8a00" : "#dfd1a7";
        ...
        {dppFormatted && (
          <svg className="-rotate-90 absolute inset-0" viewBox="0 0 56 56" fill="none">
            ...
            <circle
              ...
              strokeDashoffset={2 * Math.PI * 26 * (1 - patientWeeks / 40.5)}
            />
          </svg>
        )}
