...
export default function PatientContract({
	...
	const [headerBlocks, setHeaderBlocks] = useState<ContractHeaderBlocks | null>(
		null,
	);
	const [enterpriseHeaderBlocks, setEnterpriseHeaderBlocks] =
		useState<ContractHeaderBlocks | null>(null);
	const [personalHeaderBlocks, setPersonalHeaderBlocks] =
		useState<ContractHeaderBlocks | null>(null);
	const [savedParties, setSavedParties] = useState<ContractHeaderBlocks | null>(
		null,
	);
	...
	const { execute: fetchContract, isExecuting: isLoadingFetchContract } =
		useAction(getPatientContractAction, {
			onSuccess: ({ data }) => {
				...
				if (data?.personalHeaderBlocks)
					setPersonalHeaderBlocks(data.personalHeaderBlocks);
	...
	const { executeAsync: signContractAsync, isExecuting: isSigning } = useAction(
		signPatientContractAction,
		{
			...
			onError: ({ error }) =>
				toast.error(error.serverError ?? "Erro ao assinar contrato"),
		},
	);

	const { executeAsync: getDownloadUrl } = useAction(
		getDocumentDownloadUrlAction,
	);

	const { execute: deactivateContract, isExecuting: isDeactivating } =
		useAction(deactivatePatientContractAction, {
			onSuccess: () => {
			...
			onError: ({ error }) =>
				toast.error(error.serverError ?? "Erro ao excluir contrato"),
		});

	const {
		executeAsync: createBaseFromPatientAsync,
		isExecuting: isCreatingTemplate,
	} = useAction(createBaseContractFromPatientAction, {
		onSuccess: () => {
	...
	const handleSelectBaseTemplate = (id: string) => {
		...
		setHeaderBlocks(
			enterpriseMatch ? enterpriseHeaderBlocks : personalHeaderBlocks,
		);
		setFieldErrors({});
		setMode("editing");
	};
			...
			<div className="space-y-3 px-1 pt-2">
				...
				<div className="mb-6 grid grid-cols-1 grid-cols-4 gap-4">
					<div className="col-span-3 space-y-2">
						...
						/>
						{fieldErrors.city && (
							<p className="text-destructive text-sm">{fieldErrors.city}</p>
						)}
					</div>
				...
				<div className="flex justify-end gap-2">
					<Button
						variant="ghost"
						disabled={isSigning}
						onClick={handleCancelContractForm}
					>
						Cancelar
					</Button>
					<Button
					...
					<div className="flex items-start gap-2">
						...
						<div className="space-y-1">
							...
							<p className="text-muted-foreground text-xs leading-snug">
								Um modelo de contrato pode ser reutilizado para outras
								pacientes.
							</p>
						</div>
					</div>
...
function ContractDocument({
	...
	return (
		<div
			className={cn(
				!isEditing && "flex overflow-x-auto rounded-md bg-muted/30 py-4",
			)}
		>
			<div
				className={cn(
					!isEditing && "w-[794px] shrink-0 rounded-md bg-white shadow-md",
				)}
			>
				<div
					className={cn(
					...
					{children ?? (
						<div
							...
							dangerouslySetInnerHTML={{
								__html:
									clausesHtml ||
									"<p><em>Nenhuma cláusula adicionada ainda.</em></p>",
							}}
						/>
					)}
