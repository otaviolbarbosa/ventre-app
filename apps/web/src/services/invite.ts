...
type SupabaseAdminClient = Awaited<
	ReturnType<typeof createServerSupabaseAdmin>
>;
...
export async function respondToInvite(
	...
	if (action === "accept") {
		...
		if (existingMember) {
			await supabase
				...
				.eq("id", inviteId);
			throw new Error(
				`Já existe um ${professionalType} na equipe desta paciente`,
			);
		}
		...
		if (!pregnancy?.id)
			throw new Error("Paciente não possui gestação registrada");
	...
	await supabaseAdmin
		...
		.update({
			...
			professional_type:
				(profile.professional_type as ProfessionalType) ?? null,
			status: "rejeitado",
		})
		.eq("id", inviteId);
