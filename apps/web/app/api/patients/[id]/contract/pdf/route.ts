...
export async function POST(_request: Request, { params }: Params) {
  try {
    ...
    const savedParties = contract.parties_details as import("@/lib/contract-header-text").ContractHeaderBlocks | null
    ...
    if (savedParties) {
      ...
      if (headerData.type === "enterprise") {
        ...
        headerData.teamMembers = ((teamRows ?? []) as unknown[]).map((r) => {
          const u = (r as { users: { id: string; name: string | null; professional_type: string | null; email: string | null; phone: string | null } }).users
