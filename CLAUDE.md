...
- **Vercel Cron** (`vercel.json`) — hits `apps/web/app/api/cron/*` routes directly (`billing-notifications`, `billing-statuses`). Each route checks `Authorization: Bearer $CRON_SECRET`.
